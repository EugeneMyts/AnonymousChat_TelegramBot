...
public class TelegramBot extends TelegramLongPollingBot {
            ...
            long chatId = update.getMessage().getChatId();
                ...
                default:
                    if (update.hasMessage() && update.getMessage().hasText() && connections.containsKey(chatId)) {
                        sendMessage(zheka, update.getMessage().getText());
    ...
    private void search(long chatId) {
        ...
        if (find == 0 || find == chatId) {
            sendMessage(chatId, "Searching... Companion not found," +
                    " wai  ting list is empty. You have been added to the waiting list");
        } else {
    ...
    private void registerUser(Message msg) {
        if (userReposetory.findById(msg.getChatId()).isEmpty()) {
            ...
            sendMessage(msg.getChatId(), "You are already registered in Anonymous Chat!");
